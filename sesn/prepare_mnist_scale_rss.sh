...
MNIST_ROT_SCALE_DIR="${MNIST_ROT_SCALE_SHEAR_DIR:-./datasets}"
...
do 
    ...
    python prepare_datasets_rss.py --source "$MNIST_DIR" --dest "$MNIST_ROT_SCALE_SHEAR_DIR" --min_scale 0.3 --download --seed $i
    HASH="$(python prepare_datasets_rss.py --source "$MNIST_DIR" --dest "$MNIST_ROT_SCALE_SHEAR_DIR" --min_scale 0.3 --seed $i --validate)"
