...
def make_mnist_rot_scale_shear_50k(source, dest, min_rot, max_rot, min_scale, max_scale,min_shear,max_shear, download=False, seed=0, **kwargs):
    '''
    ...
    transform = transforms.RandomAffine([min_rot, max_rot], scale=(min_scale, max_scale), shear=(min_shear,max_shear), resample=3)
    ...
    dataset_path = os.path.join(dest, 'MNIST_rot_scale_shear', "seed_{}".format(seed))
    dataset_path = os.path.join(dataset_path, "rot_scale_shear_{}_{}".format(min_scale, max_scale))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_rot', type=float, default=-180,#required=True,
                        help='min rot for the generated dataset')
    parser.add_argument('--max_rot', type=float, default=180,
                        help='max rot for the generated dataset')
    parser.add_argument('--min_shear', type=float, default=-45,#60,#80,#required=True,
                        help='min shear for the generated dataset')
    parser.add_argument('--max_shear', type=float, default=45,#60,#80,
                        help='max shear for the generated dataset')
    ...
    if args.validate:
        ...
        dataset_path = os.path.join(dest, 'MNIST_rot_scale_shear', "seed_{}".format(args.seed))
        dataset_path = os.path.join(dataset_path,
                                    "rot_scale_shear_{}_{}".format(args.min_scale, args.max_scale))
        print(get_md5_from_source_path(dataset_path))
    else:
        for k, v in vars(args).items():
            print('{}={}'.format(k, v))

        make_mnist_rot_scale_shear_50k(**vars(args))
