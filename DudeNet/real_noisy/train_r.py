...
os.environ["CUDA_VISIBLE_DEVICES"] = "0,1,2,3,4,5,6,7"#"0"
...
def main():
    ...
    filename00 = save_dir + '_fullrecord_psnr.txt' #23
    f00 = open(filename00,'w') #23
    print(net)##23
    print(net, file = f00) ##23

    criterion = nn.MSELoss(size_average=False)
    device_ids = [0,1,2,3,4,5,6,7]#[0]
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            f00.write("[epoch %d][%d/%d] loss: %.4f PSNR_train: %.4f" % (epoch+1, i+1,patch_num, loss.item(), psnr_train)+'\n') #23
        ...
        files_source = glob.glob(os.path.join(aa, opt.test_data, '*_real.png'))
        files_source.sort()
        psnr_test = 0
