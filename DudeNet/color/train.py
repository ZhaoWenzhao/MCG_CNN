...
from models_next_wt import DudeNeXtWT
from models_next import DudeNeXt
...
parser.add_argument("--noiseL", type=float, default=75, help='noise level; ignored when mode=B')
parser.add_argument("--val_noiseL", type=float, default=75, help='noise level used on validation set')
'''
...
class sum_squared_error(_Loss):  # PyTorch 1.8.1
    """
...
def main():
    ...
    #net = DudeNeXt(channels=3, num_of_layers=opt.num_of_layers) ##23
    net = DudeNeXtWT(channels=3, num_of_layers=opt.num_of_layers) ##23
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.mode == 'S':
            prepare_data(data_path='/home/bbb/data/denoi_color', patch_size=41, stride=10, aug_times=4) #tcw201810102244
            #prepare_data(data_path='data', patch_size=50, stride=40, aug_times=1) #tcw201810102244
        if opt.mode == 'B':
            prepare_data(data_path='/home/bbb/data/denoi_color', patch_size=41, stride=10, aug_times=4)
    main()
